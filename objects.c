...
Object *make_obj(char *title, char *design[], Point start, enum object_type type, int model_size, int color, int timer)
{
        ...
        for (int j = 0; j < len; ++j)
        {
            ...
            if (design[i][j] != ' ')
            {
                if (obj->color > 0)
                {
                    ...
                    mvaddch(y + (double)i, (double)(obj->position->x + j), design[i][j]);
